...
module.exports = 
  ...
  Кроме того, если вызывать buildPackage() БЕЗ package_name, то все ключи 
  ...
  ###
  Пакету можно дать и локальное имя, эта настройка так же имеет приоритет
  перед глобальной.
  Если нет ни ее ни глобальной, то все перечисленные в bundle элементы будут 
  ижектированы в глобаль
  ###
